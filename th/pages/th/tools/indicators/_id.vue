    ...
    async asyncData({ app, params, error, req, store }) {
      let locale = app.i18n.locale,
        ...
        toolData = await app.GoGoHTTP.get(
          `/api/v3/surface/tools/indicators/${params.id}`
        ),
        titleChunk,
        currentUrl
      ...
      if (toolData.code) {
        return error({ statusCode: 404, exists: toolData.exists })
      }
      let data = toolData,
      ...
      store.commit('pushBC', {
        name: i18n[locale]['1'],
        path: '/th/tools/indicators',
      })
      store.commit('pushBC', {
        name: data.name,
        path: `/th/tools/indicators/${params.id}`
      })
      return {
        id: params.id,
        data,
        ...
        linkMeta: [
          {
            rel: 'canonical',
            href: `https://www.gogojungle.co.jp/th/tools/indicators/${
              params.id
            }`,
          },
        ],
    ...
    methods: {
      ...
      getProductPassword(password) {
        return this.GoGoHTTP.post(
          '/api/v3/surface/tools/indicators/' + this.$route.params.id,
          ...
          if (this.data.isPassword === 1 && data.status == 1) {
            ...
            this.$set(this, 'linkMeta', [
              {
                rel: 'canonical',
                href: `https://www.gogojungle.co.jp/th/tools/${this.$route.type}/${
                  this.$route.id
                }`,
              },
            ])
