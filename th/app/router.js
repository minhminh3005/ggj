...
<%
function setRoute(newRoutes, oldRoutes, isChild = false) {
  let arr = []
  for (let j = 0, k = oldRoutes.length; j < k; j++) {
    const oldRoute = oldRoutes[j]
    arr.push(oldRoute)
    for (let i = 0, l = newRoutes.length; i < l; i++) {
      const newRoute = newRoutes[i]
      if (oldRoute.path !== newRoute.path) {
        continue
      }
      if (newRoute.children && oldRoute.children) {
        arr[arr.length - 1].children = setRoute(newRoute.children, oldRoute.children, true)
      }
      if (newRoute.newPath) {
        let np = newRoute.newPath,
          prefix = isChild ? '' : oldRoute.path + '/'
        if (typeof newRoute.newPath === 'string') {
          np = [np]
        }
        np.forEach(e => {
          arr.push(Object.assign({}, oldRoute, {
            path: prefix + e,
            name: oldRoute.name + encodeURIComponent(e),
            meta: { originalPath: oldRoute.path }
          }))
        })
      }
    }
...
const _components = []
const _routes = recursiveRoutes(r, '\t\t', _components)
