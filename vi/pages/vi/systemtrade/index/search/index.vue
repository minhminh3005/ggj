  ...
  <div class="o-layout flex space-between w-1000">
    <div class="o-layout__i u-670">
      ...
      <p class="c-sub-01 mt-5">
        {{ $t('2') }}：{{ params.name }}&nbsp;
        {{ reversedInfo(params.status, statuses) }}:&nbsp; {{ $t('4')
        }}{{ params.minProfitRate == 'null' ? '' : params.minProfitRate }}％～{{
          params.maxProfitRate == 'null' ? '' : params.maxProfitRate
        }}％&nbsp; {{ $t('5') }}：{{
        ...
        &nbsp;
        {{ $t('6') }}：{{
          reversedInfo(params.currencyPairs, $t('pairs'), ', ')
        }}&nbsp; {{ $t('7')
        }}{{
          reversedInfo(params.tradingStyles, $t('trading-types'), '/')
        }}
        <!-- &nbsp; {{ $t('8') }}：{{
          reversedInfo(params.technicalIndicators, $t('indicators'), '/')
        }} -->
      </p>
      <ul class="c-list-01 flex space-between mt-30 mb-25 pl-15 pr-15">
        <li
          class="c-list-01__i flex mid"
          v-for="(text, type) in sortTypes"
          :class="[`c-list-01__i-${type}`, {'is-active': sort === type }]"
      ...
      <ProductRankHorizontal05
        class="mb-10"
        v-for="(item, i) in dataResult.data"
        :product="item"
        :key="i"
      />
      <div class="text-center mt-20 mb-50">
        <paging
          :cur-page="dataResult.currentPage"
    ...
    <div class="o-layout__i u-300">
      <div class="o-cond flex layout-col mid mb-20 pb-20">
        <div class="o-cond__h flex mid w-full">
          ...
          <h2 class="mt-10 mb-10">
            {{ $t('18') }}
          </h2>
        </div>
        <input
          type="text"
          class="c-keyword mt-20 mb-5"
          :placeholder="$t('2')"
          v-model="params.name"
        />
        <div class="c-row-01 flex mid">
          <radio
            ...
            :attrs="{ id: 'status' + key, value: key }"
            :label="label"
            v-for="(label, key) in statuses"
            :key="key"
          />
        </div>
        <button class="c-btn-01 pos-rel" @click="Search(true)">
          {{ $t('25') }}
        </button>
        ...
        <hr class="c-hr-01 w-full" />
        <button class="c-btn-02 flex mid center" @click="openCurrencyBox">
          <external-link class="mr-5" />{{ $t('26') }}
        </button>
        <GogoModal
          :show="isShowModal"
          :cancel-text="null"
          ...
          :height="550"
        >
          <div slot="title" class="c-modal__t"> ┃{{ $t('27') }} </div>
          <div slot="modal-body" class="c-modal__b flex flex-wrap">
            <checkbox
              v-model="params.currencyPairs"
              :attrs="{ id: 'currencyPairs' + key, value: key }"
              :label="label"
              v-for="(label, key) in $t('pairs')"
              :key="key"
            />
          </div>
        </GogoModal>
        ...
        <div>
          ...
          <div class="c-row-03 flex flex-wrap">
            <checkbox
              ...
              :attrs="{ id: 'platformId' + key, value: key }"
              :label="label"
              v-if="key != 5"
              v-for="(label, key) in $t('platforms')"
              :key="key"
            />
          </div>
        </div>
        ...
        <div>
          ...
          <div class="c-row-01 flex mid">
            <checkbox
              ...
              :attrs="{ id: 'categoryId1' + key, value: key }"
              :label="label"
        ...
        <div>
          ...
          <div class="c-row-03 flex flex-wrap">
            <checkbox
              v-model="params.tradingStyles"
              :attrs="{ id: 'tradingStyles' + key, value: key }"
              :label="label"
              v-for="(label, key) in $t('trading-types')"
              :key="key"
            />
          </div>
        </div>
        ...
        <div>
          ...
          <div class="c-row-04 flex flex-wrap">
            <checkbox
              v-model="params.technicalIndicators"
              :attrs="{ id: 'technicalIndicators' + key, value: key }"
              :label="label"
              v-for="(label, key) in $t('indicators')"
              :key="key"
            />
          </div>
        </div> -->
...
const obj = Object.assign(
  {
    ...
    async asyncData({ app, query, req, store, route, redirect }) {
      if (
        req.originalUrl.includes('/vi/search/detail') ||
        req.originalUrl.includes('/vi/search/easy')
      ) {
        redirect(301, '/vi/systemtrade/search')
        return
      }
      let params = {
          name: null,
        ...
        linkMeta: [
          {
            rel: 'canonical',
            href: query.name
              ? `${process.env.GOGO_URL}/vi/systemtrade/search/?name=${query.name}`
          ...
          },
          {
            ...
            hreflang: 'en',
            href: query.name
              ? `${process.env.GOGO_URL}/en/systemtrade/search/?name=${query.name}`
          ...
          },
          {
            ...
            hreflang: 'th',
            href: query.name
              ? `${process.env.GOGO_URL}/th/systemtrade/search/?name=${query.name}`
