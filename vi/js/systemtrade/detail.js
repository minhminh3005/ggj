...
export default {
  ...
  methods: {
    ...
    getDataChart() {
      ...
      this.GoGoHTTP.get(
        `/api/v3/surface/systemtrade/${this.id}/chart?year=${this.yearSelect}`
      )
        .then(data => {
    ...
    onGoToRegister() {
      location.href = `/vi/register?u=/systemtrade/${this.category}/${this.id}`
    },
    onClose() {
      this.isShowModal = false
    },
    onGoToBacktest() {
      location.href = `/vi/systemtrade/${this.category}/${this.id}/backtest`
    }
  },
}
