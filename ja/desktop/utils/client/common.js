...
function youtubeParser(url, autoplay = 0) {
  ...
  if (url.includes('?')) {
    ...
    for (const i in query) {
      if (query[i] && query[i].includes('=')) {
        const q2 = query[i].split('&')
        q2.forEach(el => {
          if (!el.includes('v=')) {
            q.push(el)
          }
  ...
  if (q.length) {
    for (const i in q) {
      if (q[i]) {
        const q2 = q[i]
        if (autoplay) {
          search.push('autoplay=1')
        } else if (q2.includes('autoplay')) {
          search.push(autoplay ? 'autoplay=1' : q2)
        } else {
          search.push(q2)
        }
      }
  ...
  if (autoplay) {
    return _match
      ? `https://www.youtube.com/embed/${_match[1]}?${search}`
      : url
  } else {
    return _match ? `https://www.youtube.com/embed/${_match[1]}?${search}` : url
  }
}
