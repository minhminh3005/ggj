    ...
    <div style="width: 470px;">
      ...
      <div class="mt-15 flex">
        ...
        >
          ...
          <Lock class="lock-icon ml-5" v-if="data.isNda" />
          <Check class="check-icon ml-5" v-if="data.isNda" />
        </button>
        <button
          :class="`mr-5 flex mid btn btn-default btn-sm certificate-btn space-between ${data.isIdentity ? 'have-data' : 'none-data'}`"
        >
          <span>{{ data.isIdentity ? $t('identity') : $t('29') }}</span>
          <User2 class="lock-icon ml-5" v-if="data.isIdentity" />
          <Check class="check-icon ml-5" v-if="data.isIdentity" />
        </button>
      </div>
