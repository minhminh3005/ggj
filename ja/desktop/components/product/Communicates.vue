<template>
  <div>
    <a class="communicates flex pt-10 pb-10 space-between cursor-pointer"
      ...
      <div class="p-10 fs-12 talkbubble">
        <div class="flex space-between">
          <b class="user-name">{{ item.userName || $t('1') }}</b>
...
export default {
  ...
  methods: {
    handleClick(item) {
      let c = `${item.id},${item.isTopic},${item.topicId}`
      setCookie('goCommu', c, 5, '/systemtrade')
