    ...
    <div class="navi-info w-full">
      ...
      <div class="content-wrapp flex space-between">
        ...
        <div class="content-info flex space-between">
          ...
          <a :href="`${naviDetailUrl}articles/${item.id}`" v-if="item.isPaidContent || item.isPurchased">
            <div class="status text-center deactive" v-if="item.isPurchased">{{ $t('4') }}</div>
            <div v-else class="flex mid">
              <Price v-if="item.price" :price="item.discountPrice||item.price" :old-price="item.discountPrice ? item.price: null" class="fs-12" />
              <span class="monthly fs-12" v-if="item.isMonthlyPrice && item.price">&nbsp;/&nbsp;<b>{{ $t('6') }}</b> </span>
            </div>
          </a>
        </div>
      </div>
...
/**
 * The NaviHorizontal04.
 */
export default {
  props: {
    /**
     * The NaviHorizontal04 item.
     */
    item: {
      type: Object,
      default() {
