<template>
  ...
  <section v-if="page === 1" class="w-estimate">
    ...
    <!-- screen request -->
    <div class="estimate">
      <span class="estimate-topic">{{ $t('1') }}</span>
    ...
    <div>
      ...
      <div class="estimate-c">
        ...
        <select
          ...
          @change="onChangeLargeCate"
        >
          <option value="" selected disabled hidden>{{ $t('45') }}</option>
          <option
            v-for="option in large"
            :key="option.id"
    ...
    <div>
      ...
      <div class="estimate-c">
        ...
        <select
          ...
          :class="checkDisabledforMiddle() ? 'disable-input' : ''"
          class="select-es"
          v-model="selectedRequire"
        >
          <option value="" selected disabled hidden>
            {{ checkDisabledforMiddle() ? '-' : $t('45') }}
          </option>
          <option
      ...
      <template>
        ...
        <div>
          <ImportFile
            ...
            :maxsize="5"
          />
          <div class="text-danger mt-5" v-if="checkErr"><b>{{ $t(checkErr) }}</b></div>
        </div>
      </template>
    </skeleton>
    <!-- 4 -->
  ...
  <section class="w-estimate" v-else-if="page === 2">
    ...
    <skeleton>
      ...
      <template>
        <span class="text-confirm">{{ selectedService.name || '-' }}</span>
      </template>
    ...
    <skeleton>
      ...
      <template>
        ...
        <template v-if="historyFile.length || filesUpload.length">
          <div
            class="file-item"
            v-for="item in historyFile"
            :key="`his-cfile-${item.fileId}`"
          >
            <span>
              {{ item.name }}
            </span>
    ...
    <skeleton>
      ...
      <template>
        <span class="text-confirm">{{ formatNumber(Number(budget)) }} {{ $t('currency') }}</span>
      </template>
    </skeleton>
    <skeleton>
      <template v-slot:title>
...
const obj = Object.assign(
  {
    ...
    async asyncData({ app, params, store, error, redirect, route }) {
      ...
      let { large, requirmentItems , categoryMiddleGroupByLargeId } = allCateLevel
      store.commit('pushBC', {
        name: (i18n[app.i18n.locale || 'ja']['bc-lv1'] || '').replace(
          '{name}',
          userInfo.nickName
        ),
        path: `/estimate/${params.userId}`,
      })
      return {
        ...
        linkMeta: [
          {
            rel: 'canonical',
            href: `https://www.gogojungle.co.jp/estimate/${params.userId}`,
          },
        ],
        middleList: [],
      }
    ...
    methods: {
      checkDisabledforMiddle() {
        if(!this.selectedService || (this.selectedService.id == 1 && !this.selectedSubMid)) return true
        return false
      },
      checkDisabled() {
      ...
      onChangeSubMid() {
        ...
        this.middleList = this.selectedSubMid.id === 13 ? this.categoryMiddleGroupByLargeId['3'] : this.requirmentItems
      },
      onChangeLargeCate() {
        let largeId = this.selectedService.id
        if(largeId === 1) this.subListMid = this.categoryMiddleGroupByLargeId['1']
        else this.middleList = this.categoryMiddleGroupByLargeId[largeId]
        this.selectedRequire = ""
        this.selectedSubMid = ""
      },  
      onFormError(val) {
      ...
      descriptionTemplate() {
        return this.$t('des-lv1', { name: (this.userInfo || {}).nickName })
      },
      titleTemplate() {
        return this.$t('title-lv1', { name: (this.userInfo || {}).nickName })
      },
      showHistory() {
        this.isShowHis = true
        this.scrollTop()
      ...
      submit() {
        ...
        if(this.selectedService.id !== 1 || (this.selectedService.id === 1 && this.selectedSubMid.id === 13)) {
        ...
        formData.append('budget', Number(this.budget))
        ...
        this.GoGoHTTP.post(
          `/api/share-room/estimate/request/${this.params.userId}`,
          formData
        )
          .then(() => {
            this.page = 3
