...
const obj = Object.assign({
  ...
  data() {
    ...
    return {
      titleChunk: type 
                  ? this.$t(`highpost-${type}-title`, {time}) 
                  : this.$t('highpost-title', {time}),
    }
  },
  async asyncData({params, app, store}) {
    let type = params.type,
      ...
      name = type ? i18n[app.i18n.locale][`highpost-${type}-title`] : i18n[app.i18n.locale][`highpost-title`]
    name = (name || '').replace('{time}', time)
    store.commit('pushBC', {
      name: name,
      path: `/review/highpost${type ? '/' + type : ''}${month ? '/' + month : ''}`,
    })
    return {
      rightData,
      ls,
      ...
      linkMeta: [
        {
          rel: 'canonical',
          href: `https://www.gogojungle.co.jp/review/review/highpost${type ? '/' + type : ''}${month ? '/' + month : ''}`,
        },
      ],
    }
  },
  methods: {
    ...
    pagingClick(p) {
      let type = this.$route.params.type,
        month = this.$route.params.month
      location.href = `/review/highpost${ type ? '/' + type : ''}${ month ? '/' + month : ''}/p/${p}`
    },
  }
}, title)
export default obj
