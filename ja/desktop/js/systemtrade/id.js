...
export async function getData(
  ...
  let id = params.id,
    ...
    [detail, mainChart, externalUrl, isRangePrice, hatenaCount] = await Promise.all([
      app.GoGoHTTP.get(`/api/v3/surface/systemtrade/${category}/${id}`),
      app.GoGoHTTP.get(`/api/v3/surface/systemtrade/${category}/${id}/chartv2`, {
        params: {
          accountId: query.a || 0
        }
      }),
  ...
  store.commit('pushBC', {
    name: detail.name,
    target: { name: 'systemtrade-index-' + category + '-id', params: params },
  })
  const {
  ...
  return {
    ...
    jsonLDTemplate: generateLDJson({
      title: isPassword ? detail.name : name,
      id,
