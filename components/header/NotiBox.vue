<template>
  <div>
    <no-ssr>
      <div
        class="w-1000 noti-box-ctn"
        :class="{ 'in': isOpenNoti, 'empty': !countInfo.total }"
      >
        <div class="noti-box flex layout-col">
          <div class="noti-header flex ">
            <div class="w-full flex mid pr-3">
            ...
            <span
              ...
              class="cursor-pointer flex mid"
              @click="toggleShowNotification(false)">×</span>
          </div>
          <div class="noti-list" ref="notiList">
            ...
            <template v-else>
              ...
              />
              <Loading v-if="isProcess" :style="[!notiList.length ? { height: '100%' } : '', { 'min-height': '108px' }]"/>
            </template>
          </div>
        </div>
      </div>
    ...
    <GogoModal
      ...
      height="50vh"
      width="70vw"
      @update:show="(val) => (showReviewNoti = val)"
    >
      <div slot="modal-header" />
      <div
        slot="modal-body"
        class="bg-white flex layout-col h-full"
      >
        <div class="modal-header flex space-between pos-rel">
          <strong v-html="itemModal.subject" />
        ...
        >
          <div v-html="itemModal.content" class="content-ctn" />
        </div>
      </div>
      <div slot="modal-footer" class="text-center" style="background: #f0f0f0; padding: 14px 0;">
        <button
          class="btn btn-default"
          @click="showReviewNoti = false">{{ $t(8) }}</button>
      </div>
    </GogoModal>
  </div>
</template>
  ...
  methods: {
    ...
    toggleShowNotification(val = null) {
      ...
      if(this.isOpenNoti) {
        ...
        // close, the new notifications has been reviewed
        this.onReview()
        this.$parent.currentIcon = {}
      }
      setTimeout(() => this.onOffClickOutSide(this.isOpenNoti), 0)
    },
    onOffClickOutSide(val) {
      if(val) {
    ...
    clickOutSideHandler(e) {
      this.$nextTick(() => {
        ...
        if (!this.$el.contains(e.target)) {
          this.toggleShowNotification(false)
        }
      })
    },
    onSearchNoti() {
