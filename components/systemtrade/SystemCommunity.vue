<template>
  <div id="community-area">
    <div class="community-post p-20">
      ...
      <textarea class="text-area fs-12 w-full resize-vertical p-10 mt-5" v-model="commentText" :placeholder="$t('3')" maxlength="2000"></textarea>
    ...
    <div v-if="!loading" class="communities mt-30 p-20" id="comment-info">
      ...
      <SystemUserMessage v-for="commu in reviewData.data" :message="commu" :key="'commu-'+commu.id" class="mt-20 mb-40" :on-delete-topic="onDeleteTopic" :on-delete-reply="onDeleteReply" @posted="onPosted" @show-warning-modal="showWarningModal" />
      <div class="text-center mt-20">
        <paging :cur-page="reviewData.currentPage"
                :total="reviewData.lastPage"
                :from="reviewData.pagingFrom"
    ...
    >
      ...
      >
        <div class="inline-flex center-content actions mt-10">
          <strong style="font-size: 15px">{{ $t('18') }}</strong>
        </div>
      </div>
    </GogoModal>
  </div>
