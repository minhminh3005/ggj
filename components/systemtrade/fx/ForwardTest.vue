<template>
  <section id="forward-table">
    <div class="mt-30 flex mid space-between mb-20">
      <div class="flex">
        <LeftTitle :text="$t('15')" />
        <div v-if="showBannerTypeDemo" class="ml-20 mr-5">
          <a href="https://www.gogojungle.co.jp/re/XlTn3ZlNTyQ6Tfo" rel="nofollow">
            <img src="https://img.gogojungle.co.jp/affiliates/57/13/4" alt="外為ファイネスト MT4-ZERO口座開設" style="border: 0;"/>
          </a>
        </div>
<!--        https://gogojungle.backlog.jp/view/OAM-55743-->
        <span v-html="(!domesticBroker ? $t('brokers.' + demoBrokerId) : '') + $t('18')" class="ml-10"
            v-if="demoBrokerId && demoBrokerId != 203" />
      </div>
...
export default {
  ...
  computed: {
    ...
    showBannerTypeDemo() {
      ...
      if (this.demoBrokerId == 148 && ((today - targetDay) < 0)) {
        return true
      }
      return false
    }
