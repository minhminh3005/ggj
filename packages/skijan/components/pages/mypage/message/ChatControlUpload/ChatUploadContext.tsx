...
import {addEventListener, FileTypes, OptFileTypes} from './const'
...
const reducer = (state: StateType, {type, payload}: ActionType) => {
  ...
  case Action.ResetListFiles: {
    const array: Array<any> = cloneDeep(state.listFiles)
    for (let i = 0; i < array.length; i++) {
      if (
        array[i].type.includes('image') &&
        array[i].fileUrl.includes('blob')
      ) {
...
export function UploadImagesOrFilesProvider({children, optFile,onChange}: Props) {
  const initState: StateType = initialState
  initState.listFiles = []
  // @ts-ignore
  const [state, dispatch] = useReducer(reducer, initState)
  const isShowPopupError = state.isShowPopupMaxFiles
  const isShowPopupErrorMemo = useMemo(
    () => isShowPopupError,
    [isShowPopupError]
  )
  ...
  const handleResetListFiles = useCallback(
    () => {
      // @ts-ignore
      dispatch({type: Action.ResetListFiles, payload: null})
    },
    [],
  )

  useEffect(()=>{
    addEventListener(EventBusAction.HandleResetListFiles,handleResetListFiles)
    return ()=>{
      removeEventListener(EventBusAction.HandleResetListFiles,handleResetListFiles)
    }
  },[])
