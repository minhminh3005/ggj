...
export function createReducerManager(initReducers: { [name: string]: Reducer }) {
  ...
  return {
    ...
    reduce: (state: CombinedState<{ [x: string]: any }> | undefined, action: AnyAction) => {
...
export const getActionByNameSpace = (namespace: string) => {
  const slice = reducerManager.getSliceByName(namespace)
  if (slice) return slice.actions
  throw Error('not found slice by name ' + namespace)
}
